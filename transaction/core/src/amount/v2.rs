...
use crate::{
    domain_separators::{
        AMOUNT_BLINDING_DOMAIN_TAG, AMOUNT_SHARED_SECRET_DOMAIN_TAG, AMOUNT_TOKEN_ID_DOMAIN_TAG,
        AMOUNT_VALUE_DOMAIN_TAG,
    },
...
impl MaskedAmountV2 {
    ...
    /// convenient to use the other API. This API allows that the sender or
    /// recipient could selectively disclose the amount_shared_secret to a
    /// third party who can then use this API to audit the value of
...
fn get_amount_shared_secret(tx_out_shared_secret: &RistrettoPublic) -> [u8; 32] {
    ...
    // Safety: Blake2b512 is a 512-bit (64-byte) hash.
...
fn get_blinding_factors(amount_shared_secret: &[u8; 32]) -> (u64, u64, Scalar) {
    ...
    // commitment
    let kdf = Hkdf::<Sha512>::new(Some(b"mc-amount-blinding-factors"), amount_shared_secret);
