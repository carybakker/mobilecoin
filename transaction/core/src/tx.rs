...
impl TxPrefix {
    ...
    pub fn output_commitments(&self) -> Result<Vec<&CompressedCommitment>, TxOutConversionError> {
        self.outputs
            .iter()
            .map(|output| output.get_masked_amount().map(|ma| ma.get_commitment()))
            .collect()
    }
...
pub struct TxOut {
    ...
    #[prost(oneof = "VersionedMaskedAmount", tags = "1, 6")]
    #[digestible(name = "amount")]
    pub masked_amount: Option<VersionedMaskedAmount>,
...
impl TxOut {
    ...
    pub fn new_with_memo(
        ...
        let masked_amount = Some(VersionedMaskedAmount::new(
            block_version,
            amount,
            &shared_secret,
        )?);
    ...
    /// missing
    pub fn get_masked_amount(&self) -> Result<&VersionedMaskedAmount, TxOutConversionError> {
        self.masked_amount
            .as_ref()
    ...
    /// version. Maps to a conversion error if the masked amount field is
    /// missing
    pub fn get_masked_amount_mut(
        &mut self,
    ) -> Result<&mut VersionedMaskedAmount, TxOutConversionError> {
        self.masked_amount
            .as_mut()
            .ok_or(TxOutConversionError::UnknownMaskedAmountVersion)
    }
...
impl TryFrom<&TxOut> for ReducedTxOut {
    ...
    fn try_from(src: &TxOut) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            commitment: *src.get_masked_amount()?.get_commitment(),
        })
    }
}
